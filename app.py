...
st.markdown(
    """
    **Instrucciones**:
    1. Sube **DATOS CLIENTES** (Excel con todos los datos).
    2. Sube **FR-EI-02** (Word: ENTREGA DE EQUIPO).
    3. Sube **FR-EI-04**, **FR-EI-03**, **FR-EI-05** (Excels: HOJA DE VIDA, PROTOCOLO, CRONOGRAMA).
    4. Haz clic en **Generar** y descarga el ZIP.
    """
)
st.markdown("---")
...
fr_ei_04_file = st.file_uploader("📑 FR-EI-04 HOJA DE VIDA DEL EQUIPO (Excel)", type=["xlsx"])
fr_ei_03_file = st.file_uploader("📑 FR-EI-03 PROTOCOLO DE MANTENIMIENTO (Excel)", type=["xlsx"])
cronograma_file = st.file_uploader("📑 FR-EI-05 CRONOGRAMA DE MANTENIMIENTO (Excel)", type=["xlsx"])

# ==============================
# Helpers
# ==============================
def write_respecting_merged(ws, target_cell, value):
    """Escribe en la celda superior izquierda si pertenece a un merge."""
...
if not (plantilla_datos_file and fr_ei_02_file and fr_ei_04_file and fr_ei_03_file and cronograma_file):
    st.info("Sube todos los archivos para habilitar la generación.")
    st.stop()

# ==============================
# Leer plantilla datos
# ==============================
try:
    ...
    st.error(f"No se pudo leer DATOS CLIENTES: {e}")
    st.stop()
...
base_name = st.text_input("Nombre base del ZIP", value="DOCUMENTACION_CLIENTES")
...
if generate:
    with st.spinner("Generando archivos..."):
        ...
        for index, row in df.iterrows():
            try:
                cliente = safe_str(row.get("CLIENTE", "")).replace(" ", "_")
                equipo = safe_str(row.get("NOMBRE DEL EQUIPO", "")).replace(" ", "_")
                ...
                ubicacion = safe_str(row.get("UBICACIÓN DEL EQUIPO (ÁREA)", ""))
                ...
                # Carpeta temporal cliente-equipo
                ...
                try:
                    table = doc.tables[0]
                    table.cell(0, 1).text = row.get("NOMBRE DEL EQUIPO", "")
                    table.cell(1, 1).text = referencia
                    table.cell(2, 1).text = serie
                    table.cell(3, 1).text = fecha_word
                except:
                    pass
                doc.save(os.path.join(folder_path, f"FR-EI-02-{cliente}-{equipo}.docx"))

                # ---------- B) FR-EI-04 (Excel Hoja de vida) ----------
                ...
                write_respecting_merged(ws_crono, "D6", ubicacion)
        ...
        st.download_button("⬇️ Descargar ZIP", data=data_bytes, file_name=f"{base_name}.zip", mime="application/zip")
